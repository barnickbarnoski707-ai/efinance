# 该文件包含 tool/stock/getter.py 模块的相关功能实现，提供给外部调用。
import calendar
import json
import signal
import sys
...
def get_quote_history(
    ...
    **kwargs,
    ...
    >>> # 关键字查询'PG'，限定股市为A股
    >>> from tool.utils import MarketType
...
def get_latest_quote(stock_codes: Union[str, List[str]], **kwargs) -> pd.DataFrame:
    ...
    Notes
    -----
    当需要获取多只沪深 A 股 的实时涨跌情况时，最好使用 ``tool.stock.get_realtime_quotes``
...
def get_all_company_performance(date: str = None) -> pd.DataFrame:
    ...
    你也可以通过函数 ``tool.stock.get_all_report_dates`` 来获取可选日期
