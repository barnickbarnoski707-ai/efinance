# 该文件包含 tool/futures/getter.py 模块的相关功能实现，提供给外部调用。
...
def get_futures_base_info() -> pd.DataFrame:
    ...
    Notes
    -----
    这里的 行情ID 主要作用是为使用函数 ``tool.futures.get_quote_history``
    获取期货行情信息提供参数
    """
...
def get_realtime_quotes() -> pd.DataFrame:
    ...
    Notes
    -----
    如果不记得行情ID,则可以调用函数 ``tool.futures.get_realtime_quotes`` 获取
    接着便可以使用函数 ``tool.futures.get_quote_history``
    来获取期货 K 线数据
...
def get_deal_detail(quote_id: str, max_count: int = 1000000) -> pd.DataFrame:
    ...
    Notes
    -----
    行情ID 格式参考 ``tool.futures.get_futures_base_info`` 中得到的数据
